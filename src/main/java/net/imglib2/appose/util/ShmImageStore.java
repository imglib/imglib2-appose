...
public final class ShmImageStore implements AutoCloseable
{
	...
	private record Entry(
			ShmImg< ? > img,
			Class< ? > typeClass,
			Dimensions dimensions ) implements AutoCloseable
	{
		private Entry
		{
	 ...
	 * @return the existing or newly allocated shared-memory image.
	 */
	public < T extends NativeType< T > > ShmImg< T > ensure(
			final String name,
			final T type,
	...
	/**
	 * Copies an input image into a named shared-memory image.
	 * <p>
	 ...
	 */
	public < T extends NativeType< T > > ShmImg< T > putInput( final String name, final RandomAccessibleInterval< T > input )
	{
	...
	/**
	 * Copies a named shared-memory image into the specified destination image.
	 *
	 * @param name
	 *            buffer name.
	 * @param destination
	 ...
	 *             do not match.
	 */
	public < T extends NativeType< T > > void copyTo( final String name, final RandomAccessibleInterval< T > destination )
	{
	 ...
	 * <p>
	 * You probable should not call this or use the output of this method
	 * directly. The output is a {@link ShmImg} which is a shared memory image,
	 * that can be closed by another thread. It is best to use
	 ...
	 * <p>
	 * TODO: Consider removing or leave for debug.
	 *
	 * @param name
	 *            buffer name.
	 ...
	 */
	@SuppressWarnings( "unchecked" )
	public < T extends NativeType< T > > ShmImg< T > get( final String name )
	{
	...
	/**
	 * Returns the named shared-memory image without generic type checking.
	 * <p>
	 * You probable should not call this or use the output of this method
	 * directly. The output is a {@link ShmImg} which is a shared memory image,
	 * that can be closed by another thread. It is best to use
	 ...
	 *             if no buffer with the given name exists.
	 */
	public ShmImg< ? > getRaw( final String name )
	{
		final Entry entry = entries.get( name );
		if ( entry == null )
			throw new IllegalArgumentException( "No shared-memory image named '" + name + "'." );
	...
	/**
	 * Returns the named shared-memory image, or {@code null} if absent.
	 * <p>
	 * You probable should not call this or use the output of this method
	 * directly. The output is a {@link ShmImg} which is a shared memory image,
	 * that can be closed by another thread. It is best to use
	 ...
	 * @param name
	 *            buffer name.
	 * @return the shared-memory image, or {@code null}.
	 */
	public ShmImg< ? > getRawOrNull( final String name )
	{
		final Entry entry = entries.get( name );
		return entry == null ? null : entry.img();
	}
	 ...
	 * from the output and input images, respectively.
	 * <p>
	 * TODO: Consider removing or lowering visibility.
	 *
	 * @return a new map from buffer names to {@link ShmImg} instances.
	 */
	public Map< String, Object > asApposeMap()
	{
		final Map< String, Object > map = new LinkedHashMap<>();

		for ( final var entry : entries.entrySet() )
			map.put( entry.getKey(), entry.getValue().img() );
